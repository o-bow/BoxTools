...
class GitPythonCoreTools:
    ...
    def is_branch_on_remote(self, git_object, origin_name: str = 'origin', branch_name: str = None):
        ...
        self.LOGGER.show_command_log(f'git fetch {origin_name}')
        git_object.execute(['git', 'fetch', origin_name])
        self.LOGGER.show_command_log('git branch -r')
        branches = git_object.execute(['git', 'branch', '-r']).splitlines()
        remote_branch = f'{origin_name}/{branch_name}'
        return any(remote_branch == b.strip() for b in branches)
